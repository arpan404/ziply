            ...
            if (params[i][0] == '-') {
                if (availableArguments.find(params[i]) != availableArguments.end()) {
                    if (i + 1 < argumentsListLength) {
                        if (availableArguments.find(params[i + 1]) == availableArguments.end() &&
                            params[i + 1][0] == '-') {
                            displayEnteredArguments(this);
...
void Parser::displayHelpTexts() {
    ...
    std::cout
        << "  -c  <ratio>       Compression prevention (pixel-to-bit ratio). Options: 1[default], 2, 4, 8, 16.\n"
