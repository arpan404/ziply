...
std::vector<fs::path> file::getConvertFilePath(const std::string &inputFileName, const std::string &outputFileName) {
    ...
    if (file::pathExists(inputFilePath)) {
        ...
        throw Error("Input file '" + inputFileName + "' does not exist.", "file-e-z1"); // Throw an error if the file does not exist
    }
    ...
    if (file::pathExists(outputFilePath)) {
        throw Error("Output file '" + outputFileName + "' already exists.", "file-e-z2"); // Throw an error if the file already exists
    }
